	...
	<div class="app-contain" :style='{"minHeight":"100vh","padding":"0 0 40px","margin":"0 auto","overflow":"hidden","borderRadius":"4px","background":"none","width":"64%","position":"relative"}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('zhenliaoxiangmu','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
			...
			<div class="data_box">
				<div class="data_view">
					<div class="data_item animation_box" v-for="(item,index) in list" :key="index" @click.stop="detailClick(item.id)" >
						<div class="data_img_box" v-if="item.xiangmutupian && item.xiangmutupian.substr(0,4)=='http'" @click.stop="preViewClick(item.xiangmutupian)">
							<el-image class="data_img" :src="item.xiangmutupian" fit="cover"></el-image>
						</div>
						<div class="data_img_box" v-else @click.stop="preViewClick($config.url+item.xiangmutupian.split(',')[0])">
							<el-image class="data_img" :src="item.xiangmutupian?$config.url + item.xiangmutupian.split(',')[0]:''"
								fit="cover"></el-image>
						</div>
						<div class="data_content">
							<div class="data_title">{{item.xiangmumingcheng}}</div>
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/zhenliaoxiangmuAdd')
	}
	//判断是否从个人中心跳转
	...
	const detailClick = (id) => {
		router.push('zhenliaoxiangmuDetail?id=' + id + (centerType.value?'&&centerType=1':''))
	}
	//下载文件
	const download = (file) =>{
		if(!file){
