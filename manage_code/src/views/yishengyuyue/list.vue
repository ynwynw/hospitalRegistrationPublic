	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('yishengyuyue','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('yishengyuyue','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('yishengyuyue','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('yishengyuyue','删除')">删除</el-button>
					<el-button type="warning" @click="echartClick1" v-if="btnAuth('yishengyuyue','预约人数统计')">预约人数统计</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('yishengyuyue','查看')" @click="infoClick(scope.row.id)">详情</el-button>
						<el-button v-if="btnAuth('yishengyuyue','就诊')" type="success" @click="yishengjiuzhenCrossAddOrUpdateHandler(scope.row,'cross','','jiaohaozhuangtai','已叫号','已叫号,未叫号'.split(',')[0])">就诊</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
		...
		<!-- 统计图弹窗 -->
		<el-dialog v-model="echartVisible" :title="'预约人数统计'" width="70%">
			<div  id="renshuEchart1" style="width:100%;height:600px;"></div>
		...
		<yishengjiuzhenFormModel ref="yishengjiuzhenFormModelRef" @formModelChange="formModelChange"></yishengjiuzhenFormModel>
	</div>
</template>
<script setup>
	...
	const getList = () => {
		...
		if(searchQuery.value.renshustart){
			params['renshustart'] = searchQuery.value.renshustart
		}
		if(searchQuery.value.renshuend){
	...
	const echartClick1 = ()=>{
		...
		nextTick(()=>{
			var renshuEchart1 = echarts.init(document.getElementById("renshuEchart1"),'macarons');
			context?.$http({
				url: `yishengyuyue/value/yuyueshijian/renshu/日`,
				method: 'get'
			}).then(res=>{
				...
				// 使用刚指定的配置项和数据显示图表。
				renshuEchart1.setOption(option);
				  //根据窗口的大小变动图表
				window.onresize = function() {
				    renshuEchart1.resize();
				};
			})
		})
	}
	...
	import yishengjiuzhenFormModel from '@/views/yishengjiuzhen/formModel'
	const yishengjiuzhenFormModelRef = ref(null)
    const yishengjiuzhenCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			yishengjiuzhenFormModelRef.value.init(row.id,'cross','就诊',row,'yishengyuyue',statusColumnName,tips,statusColumnValue)
		})
    }
	//初始化
	const init = () => {
